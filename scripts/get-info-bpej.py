...
if typeBPEJ == "plants":
    ...
    df.loc[[int(codeBPEJ[5])]].to_csv(config.get("Paths", "PipeBPEJinfo"), header=True, index=False, sep=";", quoting=None, encoding="utf-8", mode="w")
elif typeBPEJ == "soilDepth":
    df = pandas.read_csv(files + "/HloubkaPudySkeletovitost.csv", sep=";", encoding="utf-8")
    df.loc[[int(codeBPEJ[6])]].to_csv(config.get("Paths", "PipeBPEJinfo"), header=True, index=False, sep=";", quoting=None, encoding="utf-8", mode="w")
elif typeBPEJ == "soilUnit":
    df = None
    soil = open(files + "/HlavniPudniJednotka.csv", "r").read().split('\n')
    lineCode = int(str(codeBPEJ[2]) + str(codeBPEJ[3])) 
    ...
    file.write("Popis kódu " + str(codeBPEJ[2]) + str(codeBPEJ[3]) + "\n")
